...
public class TownyBanners extends JavaPlugin {
    ...
    String townyBannersTag = ChatColor.GOLD + "[TownyBanners] ";

    @Override
    public void onEnable() {
        BannersCommands commands = new BannersCommands();

        getCommand("tbanner").setExecutor(commands);
        getCommand("nbanner").setExecutor(commands);

        getServer().getPluginManager().registerEvents(new BannersEvents(), this);
        ...
        for (Town town : towns) {
            ...
            if (town.hasMeta("banner")) {
                ...
                try {
                    ...
                    Bukkit.getUnsafe().loadAdvancement(NamespacedKey.minecraft(townAdvancementPrefix + townName.toLowerCase()), new BannerAdvancement().getJsonAdvancement(townName, townBanner, "yellow"));
                ...
                try {
                    ...
                    if (nation.hasMeta("banner")) {
                        ...
                        try {
                            ...
                            Bukkit.getServer().reloadData();
                            Bukkit.getUnsafe().loadAdvancement(NamespacedKey.minecraft(nationAdvancementPrefix + nationName.toLowerCase()), new BannerAdvancement().getJsonAdvancement(nationName, nationBanner, "gold"));
